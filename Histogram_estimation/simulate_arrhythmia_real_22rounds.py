...
from heavy_hitters_protocol.protocol import Coordinator
...
def main() -> None:
    ...
    # Produce a 1000-round picked-id trace using the same protocol sampler
    # (same seed and config) so rounds continue consistently beyond estimation.
    picks_coordinator = Coordinator(
        n_clients=n_clients,
        n_decryptors=n_clients,
        labels=labels,
        clipping_threshold=CLIPPING_THRESHOLD,
        epsilon=EPSILON,
        gamma=gamma,
    ...
    picks_result = picks_coordinator.run_protocol(
        train_client_histograms, return_detailed=True
    )
    picked_ids_1000 = picks_result.client_sets_per_round
