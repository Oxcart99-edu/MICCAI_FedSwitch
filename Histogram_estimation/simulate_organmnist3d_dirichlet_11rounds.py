...
Setup:
- 50 clients
- Client size: min samples each + remaining from Dirichlet(alpha=0.5)
- Label distributions per client from Dirichlet(alpha in {0.1, 0.5, 1.0})
- Train target built from train+val counts (test kept as holdout reference)
- Generate picked IDs from scratch for 1000 rounds (seed=42)
- Estimation uses first 11 rounds of picked IDs
...
def save_client_samples_histogram(
    ...
    ax.set_title(f"OrganMNIST3D Client Sample Count Histogram (label alpha={label_alpha})")
...
def save_client_label_stacked_bars(
    ...
    ax.set_title(
        f"OrganMNIST3D Per-Client Label Composition (label alpha={label_alpha}, "
...
def save_picked_ids_and_estimate(
    ...
    payload = {
        ...
        "picked_ids_by_round": [
            ...
            for i, ids in enumerate(picked_ids_by_round)
        ],
...
def main() -> None:
    ...
    output_dir = base_path / "results" / "organmnist3d_dirichlet_11rounds"
    ...
    print("=" * 80)
    print("ORGANMNIST3D DIRICHLET CLIENT SIMULATION (3 LABEL SCENARIOS, TRAIN+VAL)")
    print("=" * 80)
    ...
    for label_alpha in LABEL_ALPHAS:
        ...
        dataset_config = DatasetConfig(
            name=f"organmnist3d_dirichlet_alpha_{label_alpha}",
            n_clients=N_CLIENTS,
            labels=LABELS,
            ground_truth=true_hist,
            data_path=Path("synthetic_organmnist3d"),
            gamma=gamma,
            rounds=N_ROUNDS,
        ...
        runner.save_results(
            [result], f"organmnist3d_dirichlet_alpha_{label_alpha}_results.json"
        )
    ...
    summary_path = output_dir / "organmnist3d_dirichlet_11rounds_summary.json"
    with open(summary_path, "w") as f:
        json.dump(summary, f, indent=2)
