...
PICKED_IDS_SOURCE_FILE = Path(
    "results/ham10000_dirichlet_22rounds_orig/picked_ids_1000_rounds_seed42.json"
)
...
def load_picked_ids_source(
    ...
    """Load fixed picked IDs from JSON and validate shape/index bounds."""
    source_path = source_file if source_file.is_absolute() else base_path / source_file
    with open(source_path) as f:
        raw = json.load(f)

    if not isinstance(raw, list) or len(raw) < n_rounds_for_estimate:
        raise ValueError(
            f"Picked-ID source must contain at least {n_rounds_for_estimate} rounds."
        )

    all_rounds: List[List[int]] = []
    for idx, entry in enumerate(raw, start=1):
        if not isinstance(entry, dict) or "picked_ids" not in entry:
            raise ValueError(f"Invalid picked-ID entry at index {idx}.")
        ids = list(entry["picked_ids"])
        for client_id in ids:
            if client_id < 0 or client_id >= n_clients:
                raise ValueError(
                    f"Invalid client id {client_id} at round {idx} in picked-ID source."
                )
        all_rounds.append(ids)

    estimate_rounds = all_rounds[:n_rounds_for_estimate]
    for round_idx, ids in enumerate(estimate_rounds, start=1):
        if len(ids) != expected_clients_per_round:
            raise ValueError(
                f"Round {round_idx} has {len(ids)} ids, expected {expected_clients_per_round}."
            )

    return all_rounds, estimate_rounds
...
def main() -> None:
    ...
    picked_ids_1000, estimate_round_picks = load_picked_ids_source(
        base_path=base_path,
        source_file=PICKED_IDS_SOURCE_FILE,
        n_rounds_for_estimate=N_ROUNDS,
        expected_clients_per_round=CLIENTS_PER_ROUND,
        n_clients=N_CLIENTS,
    )
    ...
    print(f"Picked-ID source file: {PICKED_IDS_SOURCE_FILE}")
    print(
    ...
    for label_alpha in LABEL_ALPHAS:
        ...
        save_picked_ids_and_estimate(
            ...
            first_rounds_match_estimate=first_rounds_match_estimate,
            picked_ids_source_file=str(PICKED_IDS_SOURCE_FILE),
        )
